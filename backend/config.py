...
class Settings:
    ...
    def __init__(self):
        ...
        # Handle test environment where /data might not be writable
        try:
            self.DATA_DIR.mkdir(parents=True, exist_ok=True)
        except PermissionError:
            # Fallback to temp directory for tests
            import tempfile
            self.DATA_DIR = Path(tempfile.gettempdir()) / "makevaslim_test_data"
            self.DATA_DIR.mkdir(parents=True, exist_ok=True)
        ...
        # ── Cloudflare (OPTIONAL) ─────────────────────────────────────────────
        # All Cloudflare settings are OPTIONAL - only required if using CF features
    ...
    def cf_enabled(self) -> bool:
        """Check if Cloudflare features are enabled."""
        return bool(self.CF_API_TOKEN and self.CF_ACCOUNT_ID)

    @property
    def telegram_enabled(self) -> bool:
        """Check if Telegram bot is configured."""
        return bool(self.TELEGRAM_BOT_TOKEN and self.TELEGRAM_ADMIN_IDS)
