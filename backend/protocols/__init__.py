...
def generate_all_links(
    ...
    for port in ports:
        ...
        if str(port) not in settings.TLS_PORTS or security == "none":
            links.setdefault("shadowsocks", []).append(generate_shadowsocks(config))

        # Hysteria2 (QUIC only)
        if str(port) in settings.TLS_PORTS:
...
# ════════════════════════════════════════════════════════════════════════════════
# User-based Link Builder (for tests/compat)
# ════════════════════════════════════════════════════════════════════════════════

def build_links(user, host: str, ports: list, transport: str = "ws") -> dict:
    """Build protocol links from user object (for backward compat)."""
    uuid = getattr(user, 'uuid', None)
    label = getattr(user, 'label', 'User')
    fingerprint = getattr(user, 'fingerprint', 'chrome')
    alpn = getattr(user, 'alpn', '')
    proto = getattr(user, 'protocol', 'vless')
    
    return generate_all_links(
        uuid=uuid,
        host=host,
        ports=ports,
        remark=label,
        ...
        fingerprint=fingerprint,
        alpn=alpn,
        security="tls" if any(p in (443, 8443, 2053, 2083, 2087, 2096) for p in ports) else "none",
    )
